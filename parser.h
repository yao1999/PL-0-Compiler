...
void statement(FILE *ifp, FILE *screenOutput)
{
  ...
  switch(recent_token)
  {
    ...
    case identsym:
      ...
      emit(4, regi-1, currLevel - symbol_table[index12].level, thisAddr);
	...
	case callsym:
		...
		{
			emit(5, 0, currLevel - symbol_table[index34].level, symbol_table[index34].addr);
		}
		else
		{
			// 14. Call must be followed by an identifier.
